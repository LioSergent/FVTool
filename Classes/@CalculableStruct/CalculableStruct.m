classdef (InferiorClasses = {?CellVariable}) CalculableStruct < dynamicprops
    % Somewhat convoluted class to allow labeled calculation
    % Basically just a vector and a struct that stores what each index means in the vec
    % Not necessary but can help avoid ordering mistakes
    % Two ways of constructing: with constructor from a struct (expensive) or from_vec
    % (inexpensive)
    % Properties have to be added afterwards if need (expensive).
    % If mutating is safe, mutating is faster.
    % For now operations are recreating (with from_vec), but could be change for mutation
